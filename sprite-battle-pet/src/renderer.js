...
let currentSize = SIZE_PRESETS[DEFAULT_SIZE_NAME];
let bubbleUntil = 0;
...
function setState(stateName, durationMs = 0, options = {}) {
  ...
  currentDirection = options.direction || currentDirection;
...
function jumpArcOffset(now) {
  ...
  const progress = activeProgress(now, 940);
  const arc = Math.sin(progress * Math.PI);
  const landingBounce = progress > 0.84
    ? Math.sin((progress - 0.84) / 0.16 * Math.PI) * 8
    : 0;
  const jumpHeight = Math.min(72, Math.max(44, canvas.height * 0.22));
...
function hitShakeOffset(now) {
  ...
  const progress = activeProgress(now, 980);
...
function drawSlash(frame, centerX, centerY, scale) {
  if (currentState !== 'attack' || frame < 2 || frame > 8) return;
...
function drawHitFlash(now, x, y, width, height) {
  ...
  context.lineWidth = 2;
  context.beginPath();
  context.arc(x + width * 0.54, y + height * 0.38, Math.max(18, width * 0.22), 0, Math.PI * 2);
  context.stroke();
...
function drawFrame(now) {
  ...
  const sx = frame * manifest.frame.width + trim.x;
  const sy = trim.y;
  const petHeight = currentSize.petHeight;
  const scale = petHeight / trim.height;
  const drawWidth = Math.round(trim.width * scale);
  const drawHeight = Math.round(trim.height * scale);
  const jumpOffset = jumpArcOffset(now);
  const shake = hitShakeOffset(now);
  const x = Math.round((canvas.width - drawWidth) / 2 + shake.x);
  const baseY = Math.round(canvas.height - drawHeight - BOTTOM_INSET);
  const y = Math.round(baseY + jumpOffset + shake.y);
  const groundY = baseY + drawHeight - 3;
  const shouldMirror = currentDirection === 'left';
  const jumpProgress = currentState === 'jump' ? activeProgress(now, 940) : 0;
...
function setSize(sizeName) {
  selectedSizeName = SIZE_PRESETS[sizeName] ? sizeName : DEFAULT_SIZE_NAME;
  currentSize = SIZE_PRESETS[selectedSizeName];
...
function onPointerUp(event) {
  ...
  if (dragging) {
    ...
    if (!isDown) {
      setState('jump', 780, { message: '落地。', messageDuration: 900, suppressBubble: true });
    }
    return;
  }
...
async function start() {
  ...
  manifest = window.battlePetAPI.loadManifest();

  const paths = new Set(Object.values(manifest.states).map((spec) => spec.sheet));
  const loaded = await Promise.all(Array.from(paths).map(loadImage));
  images = Object.fromEntries(loaded);
  console.log(`battle pet assets loaded: ${Object.keys(images).length}`);
